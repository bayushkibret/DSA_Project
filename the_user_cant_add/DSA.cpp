...
private:
    sqlite3* db;
    string currentUser;
    vector<Product> products; // Stack to track recently viewed products
    vector<Order> orders; // Queue for order processing


public:
    ECommerceApp() {
        if (sqlite3_open("DSA.db", &db) != SQLITE_OK) {
    ...
    void listOrders() {
        ...
        const char* sql = "SELECT o.Order_Time, p.Product_Name "
